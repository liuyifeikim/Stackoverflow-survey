...
```{r}
# com_fun <- function(data, var, var_r, var_python){
#   var <- noquote(var)
#   var_r <- noquote(var_r)
#   var_python <- noquote(var_python)
#   data %>% tabyl(var, var_r) %>% adorn_percentages(denominator = "col") %>% select(-no) %>% rename(r = yes) -> group_r
#   data %>% tabyl(var, var_python) %>% adorn_percentages(denominator = "col") %>% select(-no) %>% rename(python = yes) -> group_python
# group_r %>% left_join(group_python) -> result
# return(result)
# }
# %>%
#   gather(key = tool, value = proportion, c(r, python)) %>%
#   ggplot(aes(var, proportion, fill = tool)) +
...
```
```{r, fig.height=8, fig.width=15}
sf_rp %>% tabyl(FormalEducation, now_r) %>% adorn_percentages(denominator = "col") %>% select(-no) %>% rename(r = yes) -> edu_r
sf_rp %>% tabyl(FormalEducation, now_python) %>% adorn_percentages(denominator = "col") %>% select(-no) %>% rename(python = yes) -> edu_python
edu_r %>% left_join(edu_python, by = "FormalEducation") %>% 
  gather(key = tool, value = proportion, c(r, python)) %>% 
...
#编程经验
```{r, fig.height=8, fig.width=15}
sf_rp %>% tabyl(YearsCoding, now_r) %>% adorn_percentages(denominator = "col") %>% select(-no) %>% rename(r = yes) -> edu_r
sf_rp %>% tabyl(YearsCoding, now_python) %>% adorn_percentages(denominator = "col") %>% select(-no) %>% rename(python = yes) -> edu_python
edu_r %>% left_join(edu_python, by = "YearsCoding") %>% 
  gather(key = tool, value = proportion, c(r, python)) %>% 
...
sf_rp %>% tabyl(YearsCodingProf, now_r) %>% adorn_percentages(denominator = "col") %>% select(-no) %>% rename(r = yes) -> edu_r
sf_rp %>% tabyl(YearsCodingProf, now_python) %>% adorn_percentages(denominator = "col") %>% select(-no) %>% rename(python = yes) -> edu_python
edu_r %>% left_join(edu_python, by = "YearsCodingProf") %>% 
  gather(key = tool, value = proportion, c(r, python)) %>% 
...
#专业类型
```{r, fig.height=8, fig.width=15}
sf_rp %>% tabyl(UndergradMajor, now_r) %>% adorn_percentages(denominator = "col") %>% select(-no) %>% rename(r = yes) -> major_r
sf_rp %>% tabyl(UndergradMajor, now_python) %>% adorn_percentages(denominator = "col") %>% select(-no) %>% rename(python = yes) -> major_python
major_r %>% left_join(major_python, by = "UndergradMajor") %>% 
  gather(key = tool, value = proportion, c(r, python)) %>% 
